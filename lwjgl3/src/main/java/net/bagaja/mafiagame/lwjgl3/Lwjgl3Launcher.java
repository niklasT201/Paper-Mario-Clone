...
// question. thanks to the fine pos i noticed this bug even more, that objects not only fire, general objects, have a problem that the player for example diesnt have. lets say i place a fire image, and then i want to place another one behind it, then the highlight effect isnt visible behind the fire image, is like the fire image burns a hole in the highlight effect. this is obvious only the case for the transparent parts of the image. this was a problem fpr the player too, but a long time ago i fixed it, and i dont remember how i did it. maybe you see it in the image too
